...
RegisterNetEvent('qb-medical:server:DoBail', function(bool, vehInfo)
    local src = source
    ...
    if bool then
        if Player.PlayerData.money.cash >= Config.BailPrice then
            ...
            TriggerClientEvent('qb-medical:client:SpawnVehicle', src, vehInfo)
        ...
        if Bail[Player.PlayerData.citizenid] then
            Player.Functions.AddMoney('cash', Bail[Player.PlayerData.citizenid], "medicalrun-bail-paid")
...
RegisterNetEvent('qb-medical:server:01101110', function(drops)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    ...
    Player.Functions.AddMoney("bank", payment, "medicalrun-salary")
    TriggerClientEvent('QBCore:Notify', src, Lang:t("success.you_earned", {value = payment}), 'success')
end)

RegisterNetEvent('qb-medicalrun:server:nano', function()
    local chance = math.random(1,100)
    if chance > 26 then return end
    local xPlayer = QBCore.Functions.GetPlayer(tonumber(source))
    xPlayer.Functions.AddItem("ifaks", 1, false)
