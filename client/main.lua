...
local AmbulanceVehBlip = nil
local medicalBlip = nil
...
local function returnToStation()
    SetBlipRoute(AmbulanceVehBlip, true)
...
local function ismedicalVehicle(vehicle)
...
local function RemovemedicalBlips()
    ClearAllBlipRoutes()
    if AmbulanceVehBlip then
        RemoveBlip(AmbulanceVehBlip)
        AmbulanceVehBlip = nil
    end

    if medicalBlip then
        RemoveBlip(medicalBlip)
        medicalBlip = nil
    end
...
local function MenuGarage()
    ...
    for k in pairs(Config.Vehicles) do
        AmbulanceMenu[#AmbulanceMenu+1] = {
            header = Config.Vehicles[k],
            params = {
                event = "qb-medical:client:TakeOutVehicle",
                args = {
                    vehicle = k
                }
            }
...
local function SetDelivering(active)
    if PlayerJob.name ~= Config.Job then return end
    Delivering = active
end
...
local function CreateZone(type, number)
    ...
    if type == "main" then
        event = "qb-medicalrun:client:PaySlip"
        label = "Payslip"
    ...
    elseif type == "vehicle" then
        event = "qb-medicalrun:client:Vehicle"
        label = "Vehicle"
        coords = vector3(Config.Locations[type].coords.x, Config.Locations[type].coords.y, Config.Locations[type].coords.z)
        heading = Config.Locations[type].coords.h
        boxName = Config.Locations[type].label
    ...
    elseif type == "stores" then
        event = "qb-medicalrun:client:Store"
        label = "Store"
    ...
    if Config.UseTarget and type == "main" then
        ...
        zoneCombo:onPlayerInOut(function(isPointInside)
            if isPointInside then
                if type == "main" then
                    TriggerEvent('qb-medicalrun:client:PaySlip')
                elseif type == "vehicle" then
                    TriggerEvent('qb-medicalrun:client:Vehicle')
                elseif type == "stores" then
...
local function CreateElements()
    AmbulanceVehBlip = AddBlipForCoord(Config.Locations["vehicle"].coords.x, Config.Locations["vehicle"].coords.y, Config.Locations["vehicle"].coords.z)
    SetBlipSprite(AmbulanceVehBlip, 225)
    SetBlipDisplay(AmbulanceVehBlip, 4)
    SetBlipScale(AmbulanceVehBlip, 0.6)
    SetBlipAsShortRange(AmbulanceVehBlip, true)
    SetBlipColour(AmbulanceVehBlip, 4)
    BeginTextCommandSetBlipName("STRING")
    AddTextComponentSubstringPlayerName(Config.Locations["vehicle"].label)
    EndTextCommandSetBlipName(AmbulanceVehBlip)

    medicalBlip = AddBlipForCoord(Config.Locations["main"].coords.x, Config.Locations["main"].coords.y, Config.Locations["main"].coords.z)
    SetBlipSprite(medicalBlip, 403)
    SetBlipDisplay(medicalBlip, 4)
    SetBlipScale(medicalBlip, 0.6)
    SetBlipAsShortRange(medicalBlip, true)
    SetBlipColour(medicalBlip, 4)
    BeginTextCommandSetBlipName("STRING")
    AddTextComponentSubstringPlayerName(Config.Locations["main"].label)
    EndTextCommandSetBlipName(medicalBlip)
...
local function GetInTrunk()
    ...
    if not ismedicalVehicle(vehicle) or CurrentPlate ~= QBCore.Functions.GetPlate(vehicle) then
    ...
    if #(pos - vector3(trunkpos.x, trunkpos.y, trunkpos.z)) > 2.5 then
    ...
    QBCore.Functions.Progressbar("work_carrybox", Lang:t("mission.take_box"), 2000, false, true, {
        ...
        TriggerEvent('animations:client:EmoteCommandStart', {"medbag"})
        hasBox = true
    end, function() -- Cancel
        isWorking = false
        StopAnimTask(ped, "anim@gangops@facility@servers@", "hotwire", 1.0)
...
local function Deliver()
    ...
    QBCore.Functions.Progressbar("work_dropbox", Lang:t("mission.deliver_box"), 2000, false, true, {
        ...
        if currentCount == CurrentLocation.dropcount then
            ...
            Delivering = false
            showMarker = false
            TriggerServerEvent('qb-medical:server:nano')
            if CurrentBlip ~= nil then
                RemoveBlip(CurrentBlip)
                ClearAllBlipRoutes()
                CurrentBlip = nil
...
RegisterNetEvent('QBCore:Client:OnJobUpdate', function(JobInfo)
    ...
    PlayerJob = JobInfo
    if OldPlayerJob == Config.Job then
        RemovemedicalBlips()
        zoneCombo:destroy()
        exports['qb-core']:HideText()
        Delivering = false
        showMarker = false
    elseif PlayerJob.name == Config.Job then
        CreateElements()
    end
end)

RegisterNetEvent('qb-medical:client:SpawnVehicle', function()
...
RegisterNetEvent('qb-medical:client:TakeOutVehicle', function(data)
    local vehicleInfo = data.vehicle
    TriggerServerEvent('qb-medical:server:DoBail', true, vehicleInfo)
    selectedVeh = vehicleInfo
end)

RegisterNetEvent('qb-medicalrun:client:Vehicle', function()
    if IsPedInAnyVehicle(PlayerPedId()) and ismedicalVehicle(GetVehiclePedIsIn(PlayerPedId(), false)) then
        if GetPedInVehicleSeat(GetVehiclePedIsIn(PlayerPedId()), -1) == PlayerPedId() then
            if ismedicalVehicle(GetVehiclePedIsIn(PlayerPedId(), false)) then
                DeleteVehicle(GetVehiclePedIsIn(PlayerPedId()))
                TriggerServerEvent('qb-medical:server:DoBail', false)
                if CurrentBlip ~= nil then
                    RemoveBlip(CurrentBlip)
                    ClearAllBlipRoutes()
                    CurrentBlip = nil
...
RegisterNetEvent('qb-medicalrun:client:PaySlip', function()
    if JobsDone > 0 then
        TriggerServerEvent("qb-medical:server:01101110", JobsDone)
        JobsDone = 0
